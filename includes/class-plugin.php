...
class Plugin
{
	...
	public function maybe_show_db_error_notice(): void
	{
		...
		echo '<div class="notice notice-error is-dismissible">'
			. '<p><strong>' . esc_html__('WP Media Manager', 'media-route-and-replace') . '</strong> — '
			. esc_html__('The plugin database table could not be created. Please check that your database user has CREATE TABLE permissions, then deactivate and reactivate the plugin.', 'media-route-and-replace')
			. '</p></div>';
	}
	...
	{
		$settings_url = admin_url('admin.php?page=media-route-and-replace');

		$links['settings'] = sprintf(
			'<a href="%s">%s</a>',
			esc_url($settings_url),
			__('Settings', 'media-route-and-replace')
		);
